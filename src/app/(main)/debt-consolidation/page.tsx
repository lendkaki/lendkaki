...
const trustBadges = [
  ...
  { icon: TrendingDown, label: "Lower Monthly Payments", sub: "Consolidate and save" },
  { icon: Shield, label: "Simplify Your Debt", sub: "One loan, one payment" },
...
const benefits = [
  ...
  {
    icon: TrendingDown,
    title: "Reduce Your Interest Costs",
    description:
      "Debt consolidation loans typically offer lower interest rates than credit cards and multiple personal loans. Save thousands over your repayment period.",
  },
  {
    icon: Banknote,
    title: "One Simple Monthly Payment",
    description:
      "Replace multiple repayment dates and amounts with a single monthly instalment. Simpler to manage and easier to budget for.",
  },
  {
    icon: HeartHandshake,
    title: "Free Matching Service",
    description:
      "Our service is 100% free for borrowers. We match you with the best debt consolidation lenders so you can focus on becoming debt-free.",
  },
];
...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "Savings vary based on your existing debts and interest rates. Credit cards in Singapore typically charge 25–28% p.a. in interest. A consolidation loan at 6–12% p.a. can save you thousands over the repayment period. Use our calculator above to estimate your savings.",
  },
  {
    question: "What types of debt can be consolidated?",
    answer:
      "Most unsecured debts can be consolidated, including credit card balances, personal loans, medical bills, and overdraft facilities. Secured debts like car loans and mortgages generally cannot be included in a standard debt consolidation plan.",
  },
  {
    question: "What are the eligibility requirements?",
    answer:
      "Generally, applicants must be at least 18 years old. We accept Singapore Citizens, Permanent Residents (PRs), and foreigners. Lenders will assess your total outstanding debt, income, and credit profile. Having a regular income makes approval more likely.",
  },
  {
    question: "Will debt consolidation affect my credit score?",
    answer:
      "In the short term, taking a new loan may cause a slight dip in your credit score due to the hard inquiry. However, consistently repaying your consolidation loan on time and reducing your overall debt load will improve your credit score over time.",
  },
  {
...
export default function DebtConsolidationPage() {
  ...
  useEffect(() => { setValue("purpose", "debt-consolidation"); }, [setValue]);

  const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycby2hR6rxThjW8CIjpMDtlWePt9HI96GUivfMMkumu1xah6fwDLjSOzHY8Kh70tIt9yj/exec";
  ...
  const onSubmit = (data: QuickLeadValues) => {
    ...
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("track", "Lead", { content_name: "Debt Consolidation Application", content_category: "Lead Form" });
    }
  };
    ...
    <>
          ...
          <div ref={formRef} id="apply" className="grid items-start gap-8 pb-12 pt-4 sm:pb-16 sm:pt-8 lg:grid-cols-[1.1fr_1fr] lg:gap-12 lg:pb-20 lg:pt-12">
            <div className="flex flex-col justify-center">
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-3xl font-bold leading-[1.15] tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">
                Consolidate Your Debts &amp; Save with{" "}
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-8 hidden items-center gap-3 lg:flex">
                <div className="flex -space-x-2">
                  {[{ src: "/testimonials/priya nair.webp", alt: "Priya Nair" }, { src: "/testimonials/tan wei ming.webp", alt: "Tan Wei Ming" }, { src: "/testimonials/ahmad rizal.webp", alt: "Ahmad Rizal" }].map((a) => (
                ...
                <span className="text-sm text-slate-900/60">Trusted by thousands of Singaporeans</span>
              </motion.div>
            </div>
              ...
              {isSuccess ? (
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl border border-border bg-white p-6 shadow-xl sm:p-8">
                  <div className="space-y-5">
                    <div>
                      <Label htmlFor="dc-name" className="mb-1.5 block text-sm font-medium text-slate-700">Name</Label>
                      <input id="dc-name" placeholder="Enter your name here" className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none transition-colors placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/20" {...register("name")} />
                    ...
                    <div>
                      ...
                      <Select defaultValue="debt-consolidation" onValueChange={(value) => setValue("purpose", value as QuickLeadValues["purpose"], { shouldValidate: true })}>
                  ...
                  <Button type="submit" disabled={isSubmitting} className="mt-6 h-12 w-full gap-2 rounded-full bg-primary text-sm font-semibold text-white shadow-lg transition-all hover:opacity-90 hover:shadow-xl">
                    {isSubmitting ? (<><Loader2 className="h-4 w-4 animate-spin" />Submitting...</>) : (<>Get My Consolidation Options<ArrowRight className="h-4 w-4" /></>)}
                  </Button>
      ...
      <section ref={benefitsRef} className="bg-background py-14 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={benefitsInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-10 text-center sm:mb-14">
            ...
            <h2 className="mt-4 text-2xl font-bold tracking-tight text-foreground sm:text-3xl lg:text-4xl">Why Choose LendKaki to Consolidate Your Debt</h2>
            <p className="mt-3 text-base text-muted-foreground sm:text-lg">We help you get debt-free faster with lower rates and simpler repayments.</p>
          </motion.div>
      ...
      <section ref={calcRef} className="bg-gradient-to-br from-primary/10 via-primary/5 to-background py-14 sm:py-20">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={calcInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Debt Consolidation Calculator</h2>
            <p className="mt-2 text-base text-muted-foreground sm:text-lg">See how much you could save by consolidating your debts.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={calcInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.15 }} className="rounded-2xl border border-border bg-white p-6 shadow-xl sm:p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                {[{ label: "Total Debt", value: fmt.format(loanAmount) }, { label: "Total Interest", value: fmt.format(calcResults.totalInterest) }, { label: "Total Payable", value: fmt.format(calcResults.totalPayable) }].map(({ label, value }) => (
      ...
      <section ref={testimonialsRef} className="bg-slate-900 py-14 sm:py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={testimonialsInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold text-white sm:text-3xl">Real People, Real Savings</h2>
            <p className="mt-2 text-sm text-slate-400 sm:text-base">Join thousands of Singaporeans who got on top of their debt with LendKaki.</p>
          </motion.div>
      ...
      <section className="bg-white py-14 sm:py-20">
        <div className="mx-auto max-w-2xl px-4 sm:px-6">
          <div className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold text-foreground sm:text-3xl">Debt Consolidation FAQs</h2>
            <p className="mt-2 text-base text-muted-foreground">Everything you need to know about debt consolidation in Singapore.</p>
          </div>
      ...
      <section ref={ctaRef} className="hero-gradient py-14 sm:py-20">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={ctaInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          <h2 className="text-2xl font-bold text-slate-900 sm:text-3xl">Ready to Take Control of Your Debt?</h2>
          <p className="mt-3 text-sm text-slate-900/70 sm:text-base">Compare debt consolidation options from 20+ lenders in 60 seconds. It&apos;s 100% free.</p>
          <Button onClick={scrollToForm} size="lg" className="mt-6 h-12 gap-2 bg-primary px-8 text-sm font-semibold text-white shadow-lg hover:bg-primary/90 sm:text-base">Compare My Options<ArrowRight className="h-4 w-4" /></Button>
        </motion.div>
      </section>
      ...
      <AnimatePresence>
        {showStickyCta && !isSuccess && (
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} transition={{ duration: 0.3 }} className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-white/95 px-4 py-3 backdrop-blur-md lg:hidden">
            <Button onClick={scrollToForm} className="h-12 w-full gap-2 bg-primary text-sm font-semibold text-white shadow-lg hover:bg-primary/90">Get My Consolidation Rates<ArrowRight className="h-4 w-4" /></Button>
          </motion.div>
        )}
      </AnimatePresence>
