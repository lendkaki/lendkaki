...
const trustBadges = [
  ...
  { icon: Clock, label: "Fast Approval", sub: "Funds within 24–48 hours" },
  { icon: Shield, label: "Secured & Unsecured", sub: "Options for every business" },
...
const benefits = [
  {
    ...
    description:
      "One application connects your business to over 20 licensed lenders and banks. Receive multiple tailored offers without the paperwork hassle.",
  },
  {
    icon: TrendingUp,
    title: "Competitive Business Rates",
    description:
      "Access business loan rates from banks and licensed financial institutions. Let lenders compete for your business funding needs.",
  },
  {
    icon: Banknote,
    title: "Fast Working Capital",
    description:
      "Get the working capital your business needs quickly. Many lenders offer rapid disbursement once your business loan is approved.",
  },
  {
    icon: BarChart3,
...
const faqItems = [
  {
    ...
    answer:
      "We connect businesses with a range of loan products including term loans, working capital loans, invoice financing, trade financing, and revolving credit facilities. The best option depends on your business needs, cash flow, and financial profile.",
  },
  {
    question: "What are the eligibility requirements for a business loan?",
    answer:
      "Generally, your business must be registered in Singapore and have been operating for at least 6–12 months. Lenders will assess your revenue, profitability, and creditworthiness. Both sole proprietors, partnerships, and Pte Ltd companies can apply.",
  },
  {
    question: "How much can my business borrow?",
    answer:
      "Business loan amounts typically range from $10,000 to $500,000 or more, depending on the lender and your business financials. Some government-assisted schemes (like the Enterprise Financing Scheme) offer higher limits. We'll match you with the best option for your situation.",
  },
  {
    question: "How quickly can my business receive the funds?",
    answer:
      "Processing times vary by lender, but many of our partners can approve and disburse funds within 24–48 hours. Having your documents ready (latest financial statements, ACRA bizfile, bank statements) will speed up the process significantly.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Yes. Many lenders on our platform are happy to work with businesses that have existing financing. We'll match you with lenders whose criteria align with your current financial situation.",
  },
];
...
export default function BusinessLoansPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors }, setError } =
  ...
  const onSubmit = (data: QuickLeadValues) => {
    ...
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("track", "Lead", {
        content_name: "Business Loan Application",
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setShowStickyCta(formRef.current.getBoundingClientRect().bottom < 0);
    };
    ...
    <>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div ref={formRef} id="apply" className="grid items-start gap-8 pb-12 pt-4 sm:pb-16 sm:pt-8 lg:grid-cols-[1.1fr_1fr] lg:gap-12 lg:pb-20 lg:pt-12">
            <div className="flex flex-col justify-center">
              ...
              >
                Singapore&apos;s Best Business Loan Rates in{" "}
              ...
              >
                One application, 20+ licensed lenders and banks. Personalised business loan offers for your company.
              </motion.p>
              ...
              >
                <div className="flex -space-x-2">
                  {[
                    ...
                    { src: "/testimonials/kenneth teo.webp", alt: "Kenneth Teo" },
                    { src: "/testimonials/tan wei ming.webp", alt: "Tan Wei Ming" },
                  ].map((avatar) => (
                    <img key={avatar.alt} src={avatar.src} alt={avatar.alt} width={32} height={32} className="h-8 w-8 rounded-full border-2 border-white object-cover" />
                  ))}
                </div>
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => <Star key={i} className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400" />)}
                </div>
                <span className="text-sm text-slate-900/60">Trusted by Singapore businesses</span>
              </motion.div>
            </div>
            ...
            >
              {isSuccess ? (
                ...
                >
                  ...
                  >
                    {[
                      { n: "1", text: "Our system is now matching your request to our network of 20+ licensed lenders." },
                      { n: "2", text: "The lender will reach out to you directly via phone call and WhatsApp with the next steps." },
                      { n: "3", text: "Answer their call and reply to their WhatsApp to speed up your loan process." },
                    ].map(({ n, text }, i) => (
                      <motion.li
                        key={n}
                      ...
                      >
                        <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/20 text-xs font-bold text-white">{n}</span>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl border border-border bg-white p-6 shadow-xl sm:p-8">
                  <div className="space-y-5">
                    <div>
                      <Label htmlFor="bl-name" className="mb-1.5 block text-sm font-medium text-slate-700">Name</Label>
                      <input id="bl-name" placeholder="Enter your name here" className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none transition-colors placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/20" {...register("name")} />
                    ...
                    <div>
                      ...
                      <Select defaultValue="business" onValueChange={(value) => setValue("purpose", value as QuickLeadValues["purpose"], { shouldValidate: true })}>
                    ...
                    <div>
                      <div className="flex items-center gap-6">
                        <label className="flex items-center gap-2">
                          <input type="radio" value="Singaporean_PR" className="h-4 w-4 border-slate-300 text-primary accent-primary" {...register("nationality")} />
                          <span className="text-sm text-slate-600">Singapore/PR</span>
                        </label>
                        <label className="flex items-center gap-2">
                          <input type="radio" value="foreigner" className="h-4 w-4 border-slate-300 text-primary accent-primary" {...register("nationality")} />
                  ...
                  <Button type="submit" disabled={isSubmitting} className="mt-6 h-12 w-full gap-2 rounded-full bg-primary text-sm font-semibold text-white shadow-lg transition-all hover:opacity-90 hover:shadow-xl">
                    {isSubmitting ? (<><Loader2 className="h-4 w-4 animate-spin" />Submitting...</>) : (<>Get Your Business Loan Rates<ArrowRight className="h-4 w-4" /></>)}
                  </Button>
                  ...
                  <div className="mt-4">
                    <label className="flex items-start gap-2.5">
                      ...
                      <span className="text-xs leading-relaxed text-slate-500">
                        ...
                        <button type="button" onClick={() => setShowPrivacy(true)} className="font-medium text-primary hover:underline">Privacy Policy</button>
      ...
      <section ref={benefitsRef} className="bg-background py-14 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={benefitsInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-10 text-center sm:mb-14">
            ...
            <h2 className="mt-4 text-2xl font-bold tracking-tight text-foreground sm:text-3xl lg:text-4xl">Why Choose LendKaki for Your Business Loan</h2>
            <p className="mt-3 text-base text-muted-foreground sm:text-lg">We make funding your business simple, fast, and free.</p>
          </motion.div>
      ...
      <section ref={calcRef} className="bg-gradient-to-br from-primary/10 via-primary/5 to-background py-14 sm:py-20">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={calcInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Business Loan Calculator</h2>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={calcInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.15 }} className="rounded-2xl border border-border bg-white p-6 shadow-xl sm:p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                {[
                  { label: "Principal", value: fmt.format(loanAmount) },
                  { label: "Total Interest", value: fmt.format(calcResults.totalInterest) },
                  { label: "Total Payable", value: fmt.format(calcResults.totalPayable) },
                ].map(({ label, value }) => (
                  <div key={label} className="rounded-xl border border-border bg-slate-50 p-4 text-center">
          ...
          <p className="mt-6 text-center text-[10px] leading-relaxed text-muted-foreground sm:text-xs">
            This calculator provides indicative estimates only and does not constitute a loan offer. Actual loan terms, interest rates, and monthly instalments may vary depending on the lender and your business profile.
          </p>
        </div>
      </section>
      ...
      <section ref={testimonialsRef} className="bg-slate-900 py-14 sm:py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={testimonialsInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold text-white sm:text-3xl">Real Results from Real Businesses</h2>
            <p className="mt-2 text-sm text-slate-400 sm:text-base">Join hundreds of Singapore businesses who secured better rates with us.</p>
          </motion.div>
      ...
      <section className="bg-white py-14 sm:py-20">
        <div className="mx-auto max-w-2xl px-4 sm:px-6">
          <div className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold text-foreground sm:text-3xl">Business Loan FAQs</h2>
            <p className="mt-2 text-base text-muted-foreground">Everything you need to know about business loans in Singapore.</p>
          </div>
          <div className="space-y-2">
            {faqItems.map((item, index) => <FaqItem key={index} question={item.question} answer={item.answer} index={index} />)}
          </div>
        </div>
      </section>
      ...
      <section ref={ctaRef} className="hero-gradient py-14 sm:py-20">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={ctaInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          <h2 className="text-2xl font-bold text-slate-900 sm:text-3xl">Ready to Fund Your Business?</h2>
          <p className="mt-3 text-sm text-slate-900/70 sm:text-base">Compare business loan rates from 20+ lenders in 60 seconds. It&apos;s 100% free.</p>
          <Button onClick={scrollToForm} size="lg" className="mt-6 h-12 gap-2 bg-primary px-8 text-sm font-semibold text-white shadow-lg hover:bg-primary/90 sm:text-base">
      ...
      <AnimatePresence>
        {showStickyCta && !isSuccess && (
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} transition={{ duration: 0.3 }} className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-white/95 px-4 py-3 backdrop-blur-md lg:hidden">
            <Button onClick={scrollToForm} className="h-12 w-full gap-2 bg-primary text-sm font-semibold text-white shadow-lg hover:bg-primary/90">
              Get My Business Loan Rates<ArrowRight className="h-4 w-4" />
            </Button>
          </motion.div>
        )}
      </AnimatePresence>
